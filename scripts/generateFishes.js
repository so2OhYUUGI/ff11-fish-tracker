...
// 1. Wikiデータのパース関数
function parseWikiData(filePath) {
    const wikiList = [];
    ...
    for (const line of lines) {
        if (!line.trim() || line.startsWith('スキル'))
            continue;
...
// 2. items.lua のパース関数
function parseItemsLua(filePath) {
    if (!fs.existsSync(filePath)) {
...
function parseItemDescriptionsLua(filePath) {
    if (!fs.existsSync(filePath))
        return {};
    const content = fs.readFileSync(filePath, 'utf-8');
    const result = {};
...
function main() {
    ...
    const fileContent = `/**
 * ============================================================================
 * [FilePath] src/data/fishes.ts
 * [Role] 全魚マスターデータ（自動生成ファイル）
 * ============================================================================
 */
