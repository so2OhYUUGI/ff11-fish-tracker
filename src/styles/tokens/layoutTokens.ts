...
export const LAYOUT_TOKENS = {
  ...
  header: {
    ...
    dropdownContainer: 'absolute right-0 mt-2 w-56 sm:w-64 bg-slate-900 border border-slate-700 rounded-lg shadow-xl py-1 z-50 divide-y divide-slate-800',
    sectionHeader: 'px-3 py-1.5 text-[10px] font-semibold tracking-wider text-slate-400 uppercase',
    dropdownItemActive: 'w-full text-left px-3 py-2 text-xs sm:text-sm flex items-center justify-between transition-colors bg-blue-600/20 text-blue-300 font-medium',
    dropdownItemInactive: 'w-full text-left px-3 py-2 text-xs sm:text-sm flex items-center justify-between transition-colors text-slate-300 hover:bg-slate-800 hover:text-white',
    dropdownActionItem: 'w-full text-left px-3 py-2 text-xs sm:text-sm text-slate-300 hover:bg-slate-800 hover:text-white flex items-center gap-2 transition-colors',
  },
  ...
  // 追従型の詳細表示パネル（サイドバー）
  // モバイル(lg未満): 画面全体の最前面にモーダル風オーバーレイ（z-50）として表示（ヘッダー高さ計算不要）
  // PC(lg以上): 右側5カラムに sticky 配置
  sidebar: {
  ...
  view: {
    ...
    // モバイル時も hidden を適用せず非隠蔽（block）にしてスクロール位置を完全に保持させる
