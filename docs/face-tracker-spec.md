 ...
 * 
 * [編集・改修時の注意事項]
 * 1. 【UI/UXの踏襲】釣魚チェッカーの3段構成カード、高密度横並びリスト、共通トークンの設計思想をそのまま適用すること。
...
| `src/features/facetracker/FaceTrackerContent.tsx` | フェイスチェッカーメイン領域の表示切替（フェイスカード・リスト・詳細表示、目標リスト・編成管理ビュー） |
| `src/features/facetracker/face/FaceCard.tsx` | 個別フェイスカード（ロール/カテゴリバッジ、3段構成レイアウト、アクセシビリティ対応） |
...
| `src/styles/components/cardStyles.ts` | 3段構成カード専用の共通スタイル定義（`CARD_STYLES`） |
...
### **カード表示 (`FaceCard`)**
- カードの垂直高さを適正に保ちつつ、情報網羅性を高める **3段構成** を採用する (`CARD_STYLES` 準拠)。
  1. **上段:** 名称表示領域（日本語名・英語名の縦並び）、ロールバッジ
  2. **中段:** 説明文領域 (`CARD_STYLES.descriptionBox`) - カテゴリおよび簡略取得方法
  3. **下段:** 属性情報・関連バッジ領域（目標リスト登録状態、マイフェイス編成登録状態）

### **リスト表示 (`FaceListItem`)**
- 垂直方向への高速スキャンと高密度表示を実現する **横並び構成** を採用する (`LIST_STYLES` 準拠)。
...
- 釣魚チェッカーの共通カード・リスト UI/UX 標準の完全踏襲。
...
- **共通スタイルトークンの参照基準**:
  - カードおよびリストの装飾・状態（デフォルト/選択中/チェック済み）には、必ず `COMMON_TOKENS`、`CARD_STYLES`、`LIST_STYLES` を使用し、ハードコードしたカラー定義を排除すること。
- **目標リスト・マイフェイス編成の制限**:
